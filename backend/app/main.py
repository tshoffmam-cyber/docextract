...
def _check_ai_keys() -> None:
    ...
    if not has_gemini and not has_anthropic:
                logger.error(
                                "ATENÇÃO: Nenhuma chave de IA configurada! "
                                "Defina GEMINI_API_KEY e/ou ANTHROPIC_API_KEY nas variáveis de ambiente. "
                                "O sistema não conseguirá processar PDFs sem pelo menos uma delas."
                )
elif not has_gemini:
            logger.warning(
                            "GEMINI_API_KEY não configurada — usando apenas Anthropic Claude como IA. "
                            "Para usar Gemini como primário, defina GEMINI_API_KEY."
            )
elif not has_anthropic:
            logger.warning(
                            "ANTHROPIC_API_KEY não configurada — sem fallback para Claude. "
                            "Se a cota do Gemini esgotar, o sistema ficará indisponível."
            )
else:
            logger.info(
...
async def config_check():
        """Endpoint de diagnóstico — mostra quais chaves estão configuradas."""
        return {
